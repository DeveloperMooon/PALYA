...
export const SideNavBar: React.FC<SideNavBarProps> = ({
  ...
  const content = (
    <>
      ...
      {/* Primary Scan Tag CTA */}
      <button
        id={isMobileDrawer ? 'mobile-drawer-scan-tag-btn' : 'sidebar-scan-tag-btn'}
      ...
      <nav className="flex-1 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
        {navItems.map((item) => {
          const isActive = currentScreen === item.id || (item.id === 'veterinary-review' && currentScreen === 'veterinary-case');
          return (
            <button
              key={item.id}
              id={`${isMobileDrawer ? 'mobile-nav-' : 'nav-'}${item.id}`}
            ...
            >
              ...
              {item.badge && (
                <span className={`text-[11px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${item.badgeColor || 'bg-surface-variant text-on-surface-variant'}`}>
                  {item.badge}
                </span>
              )}
            </button>
      ...
      <div className="pt-4 mt-2 border-t border-outline-variant/60 shrink-0">
        <div className="flex items-center gap-2 p-1.5 rounded-xl bg-surface-container-low">
          <div 
            ...
            className="flex-1 flex items-center gap-2.5 min-w-0 hover:bg-surface-container-highest p-1 rounded-lg transition-colors cursor-pointer"
